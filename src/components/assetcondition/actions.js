...
const getAssetConditionSettingsData = (params, id) => {
    return async dispatch => {
        try {
            dispatch({ type: actionTypes.GET_ASSET_CONDITION_SETTINGS_DATA_REQUEST });
            const res = await Service.getAssetConditionSettingsData(params, id);
            if (res && res.status === 200) {
                const regionData = res.data;
                if (regionData.success) {
                    dispatch({
                        type: actionTypes.GET_ASSET_CONDITION_SETTINGS_DATA_SUCCESS,
                        response: regionData
                    });
                } else {
                    dispatch({
                        type: actionTypes.GET_ASSET_CONDITION_SETTINGS_DATA_FAILURE,
                        error: regionData
                    });
                }
            } else {
...
const addAssetCondition = (id, params) => {
    return async dispatch => {
        try {
            dispatch({ type: actionTypes.ADD_ASSET_CONDITION_REQUEST });
            const res = await Service.addAssetCondition(id, params);
            if (res && res.status === 200) {
                const regionData = res.data;
                if (regionData.success) {
                    dispatch({ type: actionTypes.ADD_ASSET_CONDITION_SUCCESS, response: regionData });
                } else {
                    dispatch({ type: actionTypes.ADD_ASSET_CONDITION_FAILURE, error: regionData });
                }
            } else {
                dispatch({
                    type: actionTypes.ADD_ASSET_CONDITION_FAILURE,
                    error: res.response && res.response.data
                });
            }
        } catch (e) {
...
const getAssetConditionById = (id, tradeid) => {
    return async dispatch => {
        try {
            dispatch({ type: actionTypes.GET_ASSET_CONDITION_BY_ID_REQUEST });
            const res = await Service.getAssetConditionById(id, tradeid);
            if (res && res.status === 200) {
                const regionData = res.data;
                if (regionData.success) {
                    dispatch({
                        type: actionTypes.GET_ASSET_CONDITION_BY_ID_SUCCESS,
                        response: regionData
                    });
                } else {
                    dispatch({
                        type: actionTypes.GET_ASSET_CONDITION_BY_ID_FAILURE,
                        error: regionData
                    });
                }
            } else {
...
const updateAssetCondition = (projectId, tradeId, params) => {
    return async dispatch => {
        try {
            dispatch({ type: actionTypes.UPDATE_ASSET_CONDITION_REQUEST });
            const res = await Service.updateAssetCondition(projectId, tradeId, params);
            if (res && (res.status === 200 || res.status === 201)) {
                const building_typeData = res.data;
                if (building_typeData.success) {
                    dispatch({
                        type: actionTypes.UPDATE_ASSET_CONDITION_SUCCESS,
                        response: building_typeData
                    });
                } else {
                    dispatch({
                        type: actionTypes.UPDATE_ASSET_CONDITION_FAILURE,
