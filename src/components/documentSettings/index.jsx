...
class DocumentSettings extends Component {
    state = {
        ...
        errorMessage: "",
        dataList: [],
        paginationParams: this.props.documentSettingsReducer[this.props.match.params.settingType].entityParams.paginationParams,
        showViewModal: false,
        showWildCardFilter: false,
        selectedRowId: this.props.documentSettingsReducer[this.props.match.params.settingType].entityParams.selectedRowId,
        params: this.props.documentSettingsReducer[this.props.match.params.settingType].entityParams.params,
        selectedData: this.props.match.params.id,
        tableData: {
            //config component-field names
            keys: DocumentSettingsEntities[this.props.match.params.settingType].tableConfig.keys,
            //config contain properties of each field like isvisible ,has wild card search
            config:
                this.props.documentSettingsReducer[this.props.match.params.settingType].entityParams.tableConfig ||
                _.cloneDeep(DocumentSettingsEntities[this.props.match.params.settingType].tableConfig.config)
        },
        alertMessage: "",
        wildCardFilterParams: this.props.documentSettingsReducer[this.props.match.params.settingType].entityParams.wildCardFilterParams,
        filterParams: this.props.documentSettingsReducer[this.props.match.params.settingType].entityParams.filterParams,
        historyPaginationParams: this.props.documentSettingsReducer[this.props.match.params.settingType].entityParams.historyPaginationParams,
        historyParams: this.props.documentSettingsReducer[this.props.match.params.settingType].entityParams.historyParams,
        logData: {
            count: "",
            data: []
        },
        ...
        if (prevProps.match.params.settingType !== this.props.match.params.settingType) {
            await this.setState({
                ...
                tableData: {
                    ...
                    config:
                        this.props.documentSettingsReducer[this.props.match.params.settingType].entityParams.tableConfig ||
                        DocumentSettingsEntities[this.props.match.params.settingType].tableConfig.config
                },
                wildCardFilterParams: this.props.documentSettingsReducer[this.props.match.params.settingType].entityParams.wildCardFilterParams,
                filterParams: this.props.documentSettingsReducer[this.props.match.params.settingType].entityParams.filterParams,
                historyPaginationParams: this.props.documentSettingsReducer[this.props.match.params.settingType].entityParams.historyPaginationParams,
    ...
    refreshDataList = async () => {
        ...
        const { getDataResponse } = this.props.documentSettingsReducer[settingType] || {};
        dataList = getDataResponse[DocumentSettingsEntities[settingType].responseKey] || [];
        ...
        if (dataList && !dataList.length && paginationParams.currentPage) {
            ...
            dataList = getDataResponse[DocumentSettingsEntities[settingType].key] || [];
            totalCount = getDataResponse?.count || 0;
        }
        ...
        this.setState({
            tableData: {
                ...tableData,
                data: dataList,
                config: this.props.documentSettingsReducer[settingType].entityParams.tableConfig || tableData.config
            },
            dataList,
            paginationParams: {
                ...paginationParams,
    ...
    resetAll = async () => {
        await this.setState({
            ...
            tableData: {
                ...
                config: _.cloneDeep(DocumentSettingsEntities[this.props.match.params.settingType].tableConfig.config)
            },
            wildCardFilterParams: {},
            filterParams: {},
            selectedRowId: null
    ...
    getListForCommonFilter = async params => {
        ...
        return (
            (this.props.documentSettingsReducer[settingType].getListForCommonFilterResponse &&
                this.props.documentSettingsReducer[settingType].getListForCommonFilterResponse.list) ||
            []
        );
    };
    ...
    //function for showing edit form
    showEditPage = dataId => {
        const {
            match: {
                params: { settingType }
        ...
        let path = `/document-settings/${settingType}/edit/${dataId}`;
        addToBreadCrumpData({
            key: "edit",
            name: `Edit ${DocumentSettingsEntities[settingType].name}`,
            path
        });
        history.push(path);
    };
    //function for showing add form after add button click
    showAddForm = () => {
        const {
            match: {
                params: { settingType }
        ...
        addToBreadCrumpData({
            key: "Name",
            name: `Add ${DocumentSettingsEntities[settingType].name}`,
            path: `/document-settings/${settingType}/add`
        });
        history.push(`/document-settings/${settingType}/add`);
    };
    ...
    handleAddData = async data => {
        ...
        const { error, message } = this.props.documentSettingsReducer[settingType].addDataResponse;
        if (error) {
            this.setState(
                {
                    alertMessage: error,
    ...
    isColunmVisibleChanged = () => {
        ...
        keys.map(key => {
            if (
                !_.isEqual(config[key]?.isVisible, DocumentSettingsEntities[this.props.match.params.settingType].tableConfig.config[key]?.isVisible)
            ) {
                flag = true;
            }
        });
    ...
    // update cheyyumbo form varannnath
    handleUpdateData = async data => {
        ...
        const { updateDataResponse } = this.props.documentSettingsReducer[settingType];
        if (updateDataResponse.error) {
            ...
            this.showAlert(); //data updated then show modal
            this.cancelForm(); //after update close theupdate form
        }
    };
    ...
    getDataById = async () => {
        ...
        return this.props.documentSettingsReducer[settingType].getDataByIdResponse;
    };
    ...
    showInfoPage = (id, projectId, rowData) => {
        ...
        if (rowData) {
            addToBreadCrumpData({
                ...
                path: `/document-settings/${settingType}/info/${id}/basicdetails`
            });
            addToBreadCrumpData({
                key: "info",
                name: "Basic Details",
        ...
        history.push(`/document-settings/${settingType}/info/${id}/basicdetails`);
    };
    ...
    getLogData = async () => {
        ...
        } = this.props.documentSettingsReducer[settingType];
        if (error) {
    ...
    deleteLogOnConfirm = async () => {
        ...
        const { error, message, success } = this.props.documentSettingsReducer[settingType].deleteDataLogResponse;
        if (!success) {
    ...
    restoreLogOnConfirm = async id => {
        ...
        const { error, message, success } = this.props.documentSettingsReducer[settingType].restoreDataLogResponse;
        if (!success) {
            this.setState({ alertMessage: error || "Something went wrong !" }, () => this.showAlert());
        } else {
            this.setState({ alertMessage: message }, () => this.showAlert());
    ...
    exportTableXl = async () => {
        ...
        if (
            this.props.documentSettingsReducer[settingType].dataExportResponse &&
            this.props.documentSettingsReducer[settingType].dataExportResponse.error
        ) {
            await this.setState({ alertMessage: this.props.documentSettingsReducer[settingType].dataExportResponse.error });
            this.showAlert();
        }
    };
    ...
    //impo form close cheyyumbo vrunth
    cancelForm = () => {
    ...
    cancelInfoPage = () => {
        const { settingType } = this.props.match.params;
        popBreadCrumpOnPageClose();
        history.push(findPrevPathFromBreadCrumpData() || `/document-settings/${settingType}`);
    };

    toggleHistory = async () => {
        this.setState({ isHistory: !this.state.isHistory });
    };
    //impo data view cheythen sesham delete click cheymbo dletshowmodal print,id edukkanm
    ...
    deleteDataOnConfirm = async () => {
        ...
        const { success, message, error } = this.props.documentSettingsReducer[settingType].deleteDataResponse;
    ...
    render() {
        ...
        const selectedSetting = DocumentSettingsEntities[settingType];
