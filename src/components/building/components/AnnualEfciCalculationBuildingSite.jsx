...
class InitialFundingOptionsTable extends Component {
    ...
    render() {
        const {
            ...
            updateAnnualEfciCalculationSite,
            updateFcisSite,
            toggleLoader,
            efciBuildingData,
                                                        ...
                                                        <>
                                                            <NumberFormat
                                                                ...
                                                                onValueChange={async values => {
                                                                    const { value } = values;
                                                                    await this.props.updateAnnualEfciCalculationSite(data.id, value);
                                                                }}
                                                                onKeyPress={async event => {
                                                                    if (event.key === "Enter") {
                                                                        ...
                                                                        await toggleLoader();
                                                                        await updateFcisSite(data.id, { value: data.value });
                                                                        await this.setState({
                                                                            selectedCell: data.id,
                                                                            initialCellValue: data.value
                                                                        });
                                                                ...
                                                                onBlur={async () => {
                                                                    await updateAnnualEfciCalculationSite(data.id, this.state.initialCellValue);
                                                                    await this.setState({
                                                                        selectedCell: null,
                                                                        initialCellValue: null
                                                                    });
                                                            ...
                                                            {data.edited ?
                                                                <i className="fa fa-circle cursor-hand" aria-hidden="true"
                                                                    onClick={() => this.props.showSiteLogsTableAnnualEfci && this.props.showSiteLogsTableAnnualEfci(data.id)}
                                                                >
