...
import waterActions from "../actions";
...
class Form extends Component {
    ...
    paramFetcher = async () => {
        ...
        if (sect === "buildinginfo") {
            let {
                waterReducer: { getBuildingByIdResponse }
            } = this.props;
        ...
        if (sect === "siteinfo") {
            ...
            let {
                waterReducer: { getSiteByIdResponse }
            } = this.props;
    ...
    componentDidUpdate = (prevProps, prevState) => {
        ...
        if (currVal?.building_id !== prevState?.values?.building_id) {
            currVal?.building_id && this.handleAccountDetails({ building_id: currVal?.building_id, account_type: "Water" });
        }
        if (currVal?.account_id !== prevState?.values?.account_id) {
            currVal?.account_id && this.handleMeterDetails({ account_id: currVal?.account_id, account_type: "Water" });
        }
    };
    ...
    handleAccountDetails = async params => {
        ...
        let { accounts } = this.props.waterReducer?.getAccountsListResponse;
        this.setState({
            accounts
        });
    };
    ...
    handleRegionDetails = async id => {
        ...
        let { regions } = this.props.waterReducer?.getRegionListResponse;
        this.setState({
            regions
        });
    };
    ...
    handleSiteDetails = async (id, params) => {
        ...
        let { sites } = this.props.waterReducer?.getSiteListResponse;
        this.setState({
            sites
        });
    };
    ...
    handleBuildingDetails = async (id, params) => {
        ...
        let { buildings } = this.props.waterReducer?.getBuildingListResponse;
        this.setState({
            buildings
        });
    };
    handleMeterDetails = async params => {
        ...
        let { meters } = this.props.waterReducer?.getMeterListResponse;
        this.setState({
            meters
        });
    };
...
export default withRouter(connect(mapStateToProps, { ...waterActions })(Form));
