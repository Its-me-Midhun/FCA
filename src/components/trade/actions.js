...
const getTradeSettingsData = (params, id) => {
    return async dispatch => {
        try {
            dispatch({ type: actionTypes.GET_TRADE_SETTINGS_DATA_REQUEST });
            const res = await Service.getTradeSettingsData(params, id);
...
const exportTradeSettings = (projectId, params) => {
    return async dispatch => {
        try {
            dispatch({ type: actionTypes.GET_TRADE_EXPORT_REQUEST });
            const response = await Service.exportTradeSettings(projectId, params);
...
const getAllTradeLogs = (id,params,projectId) => {
    return async dispatch => {
        try {
            dispatch({ type: actionTypes.GET_ALL_TRADE_LOG_REQUEST });
            const res = await Service.getAllTradeLogs(id,params,projectId);
            if (res && res.status === 200) {
                const regionData = res.data;
                if (regionData.success) {
