...
class SiteReports extends Component {
    state = {
        isLoading: true,
        selectedSite: this.props.match.params.id || null,
        tabs: []
    };
    ...
    setTabs = () => {
        ...
        let siteId = this.state.selectedSite;
        let tabs = [
            {
                key: "specialReports",
            ...
            },
            {
                ...
                path: `/site/siteinfo/${siteId}/reports/reportParagraphs${tempSearch}`
            },
            {
                key: "childParagraphs",
                name: "Child Paragraphs",
                path: `/site/siteinfo/${siteId}/reports/childParagraphs${tempSearch}`
            }
        ];
        this.setState({
            tabs,
    ...
    setDynamicUrl = (key = null) => {
        ...
        switch (key) {
            case "specialReports":
                localStorage.setItem("dynamicUrl", `/site_special_reports`);
                break;
            case "reportParagraphs":
                localStorage.setItem("dynamicUrl", `/site_report_paragraphs`);
                break;
            case "childParagraphs":
                localStorage.setItem("dynamicUrl", `/site_child_paragraphs`);
                break;
            default:
                break;
        }
...
export default withRouter(SiteReports);
