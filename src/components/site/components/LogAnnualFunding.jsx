...
class LogAnnualFunding extends Component {
    constructor(props) {
        super(props);
        this.state = {
            sortOrder: null,
    ...
    sortTable = async () => {
        ...
        await this.props.sortAnnualFunding();
    }
                                                    ...
                                                    <thead>
                                                        <tr>
                                                            ...
                                                            <th className="cursor-hand"
                                                                onClick={() => logs && logs.length && this.props.sortAnnualFunding && this.sortTable()}
                                                            > Date
                                                        {logs && logs.length ?
                                                                    sortOrder === true ?
                                                            ...
                                                            logs && logs.length ?
                                                                logs.map(log =>
                                                                    <>
                                                                        <tr>
                                                                            <td>
                                                                                <b> {log.user}</b> changed {<b>amount</b>} from <b>{
                                                                                    <NumberFormat
                                                                                        prefix={"$ "}
                                                                                        displayType={"text"}
                                                                                        thousandSeparator={true}
                                                                                        value={parseInt((log && log.changeset && log.changeset.amount && log.changeset.amount[0]) || 0)}
                                                                                    />}</b> to <b>{
                                                                                        <NumberFormat
                                                                                            prefix={"$ "}
                                                                                            displayType={"text"}
                                                                                            thousandSeparator={true}
                                                                                            value={parseInt((log && log.changeset && log.changeset.amount && log.changeset.amount[1]) || 0)}
                                                                                        />}</b>
                                                                            </td>
                                                                            <td>
                                                                                {log.created_at}
...
export default LogAnnualFunding;