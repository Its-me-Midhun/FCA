...
class ChartDasboard extends Component {
    state = {
        chartArray: ["trade", "category", "building", "funding_source", "priority", "EFCI", "criticalities", "capital_types"],
        dataSource: {},
        totalAmount: 0,
        legendValues: [],
        legendArrayFunding: [],
    ...
    renderChart = type => {
        ...
        if (graphData) {
            switch (type) {
                ...
                case "criticalities": {
                    graphTypeData = graphData["criticalities"];
                    break;
                }
                case "capital_types": {
                    graphTypeData = graphData["capital_types"];
                    break;
                }
                case "system": {
                    graphTypeData = graphData["system"];
                    break;
                }
                default:
                    break;
            ...
            if (type !== "EFCI" && (type === "priority" || type === "trade" || type === "system")) {
                mergedArray &&
                    mergedArray.length &&
            ...
            if (type !== "priority" && type !== "EFCI" && type !== "trade" && type !== "system") {
                legendArray = [];
                mergedArray.forEach(function (d) {
                    if (holder.hasOwnProperty(d.name)) {
                        holder[d.name] = holder[d.name] + d.amount;
                ...
                dataSource = {
                    ...
                    plotOptions: {
                        pie: {
                            ...
                            showInLegend: this.props.dataView == "site" || this.props.dataView == "building" ? false : true,
                            legend: {
                                enabled: this.props.dataView == "site" || this.props.dataView == "building" ? false : true
                            }
                            // point: {
                            //     events: {
                ...
                return rtrnValue;
            } else if (type === "priority" || type === "trade" || type === "system") {
                let data = [];
                var holder = {};
                graphTypeData &&
                    graphTypeData.length &&
    ...
    render() {
        const { chartDisplayOrder = [] } = this.props;
        var isFullscreen = document.webkitIsFullScreen || document.mozFullScreen || false;
        let chartArray = ["trade", "system", "category", "building", "funding_source", "priority", "EFCI", "criticalities", "capital_types"];
        if (chartDisplayOrder?.length) {
            let newItems = chartArray.filter(item => !chartDisplayOrder.includes(item));
            chartArray = [...chartDisplayOrder, ...newItems];
        }
                                          ...
                                          <div className="box-order p-0">
                                              {ca != "EFCI" ? (
                                                  ...
                                                  this.renderChart(ca).dataSource.series[0].data.length ? (
                                                      <>
                                                          {" "}
                                                          <div className="hed-chart-bnr chart-mod-head chart-head-position">
                                                              <div className="right-section">
                                                                  <h3 className="mb-0 mt-0 text-left pl-3">{this.renderHeading(ca)}</h3>
