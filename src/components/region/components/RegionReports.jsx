...
class RegionReports extends Component {
    state = {
        isLoading: true,
        selectedRegion: this.props.match.params.id || null,
        tabs: []
    };
    ...
    setTabs = () => {
        ...
        let regionId = this.state.selectedRegion;
        let tabs = [
            {
                key: "specialReports",
            ...
            },
            {
                ...
                path: `/region/regioninfo/${regionId}/reports/reportParagraphs${tempSearch}`
            },
            {
                key: "childParagraphs",
                name: "Child Paragraphs",
                path: `/region/regioninfo/${regionId}/reports/childParagraphs${tempSearch}`
            }
        ];
        this.setState({
            tabs,
    ...
    setDynamicUrl = (key = null) => {
        ...
        switch (key) {
            case "specialReports":
                localStorage.setItem("dynamicUrl", `/region_special_reports`);
                break;
            case "reportParagraphs":
                localStorage.setItem("dynamicUrl", `/region_report_paragraphs`);
                break;
            case "childParagraphs":
                localStorage.setItem("dynamicUrl", `/region_child_paragraphs`);
                break;
            default:
                break;
        }
...
export default withRouter(RegionReports);
