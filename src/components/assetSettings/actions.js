    ...
    return async dispatch => {
        try {
            ...
            if (res && res.status === 200) {
                const regionData = res.data;
                if (regionData.success) {
                    dispatch({ type: actionTypes.ADD_DATA_SUCCESS, response: regionData, entity });
                } else {
                    dispatch({ type: actionTypes.ADD_DATA_FAILURE, error: regionData, entity });
                }
            } else {
                dispatch({
...
// const updateDataPiechart = (id, params, entity) => {
//     return async dispatch => {
//         try {
//             dispatch({ type: actionTypes.GET_DATA_REQUEST });
//             const res = await Service.updateDataPiechart(id, params, entity);
//             if (res && (res.status === 200 || res.status === 201)) {
//                 const building_typeData = res.data;
//                 if (building_typeData.success) {
//                     dispatch({
...
const updateDataEntityParams = (entityParams, entity) => {
    return async dispatch => {
        try {
            if (entityParams) {
                dispatch({
                    type: actionTypes.UPDATE_DATA_ENTITY_PARAMS_SUCCESS,
            ...
            dispatch({
                type: actionTypes.UPDATE_DATA_ENTITY_PARAMS_FAILURE,
                error: entityParams,
                entity
            });
        }
        ...
        try {
            ...
            if (response && response.data) {
                ...
                if (text && text.split('"')[1] === "error") {
                    dispatch({ type: actionTypes.GET_DATA_EXPORT_SUCCESS, response: { error: text.split('"')[3] }, entity });
                    return true;
                } else {
                    dispatch({ type: actionTypes.GET_DATA_EXPORT_SUCCESS, response: {}, entity });
                }
            }
            const { data } = response;
    ...
    return async dispatch => {
        try {
            ...
            if (res && res.status === 200) {
                const regionData = res.data;
                if (regionData.success) {
                    dispatch({ type: actionTypes.GET_ALL_DATA_LOG_SUCCESS, response: regionData, entity });
                } else {
                    dispatch({ type: actionTypes.GET_ALL_DATA_LOG_FAILURE, error: regionData, entity });
                }
            } else {
                dispatch({
    ...
    return async dispatch => {
        try {
            ...
            if (res && res.status === 200) {
                const regionData = res.data;
                if (regionData.success) {
                    dispatch({ type: actionTypes.RESTORE_DATA_LOG_SUCCESS, response: regionData, entity });
                } else {
                    dispatch({ type: actionTypes.RESTORE_DATA_LOG_FAILURE, error: regionData, entity });
                }
            } else {
                dispatch({
    ...
    return async dispatch => {
        try {
            ...
            if (res && res.status === 200) {
                const regionData = res.data;
                if (regionData.success) {
                    dispatch({ type: actionTypes.DELETE_DATA_LOG_SUCCESS, response: regionData, entity });
                } else {
                    dispatch({ type: actionTypes.DELETE_DATA_LOG_FAILURE, error: regionData, entity });
                }
            } else {
                dispatch({
    ...
    return async dispatch => {
        try {
            ...
            if (res && res.status === 200) {
                const regionData = res.data;
                if (regionData.success) {
                    dispatch({ type: actionTypes.DELETE_DATA_SUCCESS, response: regionData, entity });
                } else {
                    dispatch({ type: actionTypes.DELETE_DATA_FAILURE, error: regionData, entity });
                }
            } else {
                dispatch({
    ...
    return async dispatch => {
        try {
            ...
            if (res && res.status === 200) {
                const regionData = res.data;
                if (regionData.success) {
                    dispatch({ type: actionTypes.GET_DROPDOWN_SUCCESS, response: regionData, level });
                } else {
                    dispatch({ type: actionTypes.GET_DROPDOWN_FAILURE, error: regionData, level });
                }
            } else {
                dispatch({
