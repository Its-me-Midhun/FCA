...
import electricActions from "../actions";
...
class Form extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            subTemplates: [
                { key: "meter_read", type: "date", label: "Meter Read" },
                { key: "billing_days", type: "number", label: "Billing Days" },
                ...
                { key: "mmbtu_usage", type: "numberclick", label: "MMBTU usage *" },
                { key: "kw_usage", type: "number", label: "KW Demand *" },
                { key: "kwh_cost", type: "number", label: "kWh cost ($) *", prefix: true },
                { key: "kw_cost", type: "number", label: "KW cost ($) *", prefix: true }
                ...
                // { key: "ccf_cost", type: "number", label: "CCF Cost" },
            ],
            errorParams: {
                region_id: "",
                site_id: "",
            ...
            initialClientUsers: [],
            alertMessage: "Please enter kWh usage to calculate"
        };
    }
    ...
    paramFetcher = async () => {
        ...
        if (sect === "buildinginfo") {
            let {
                electricReducer: { getBuildingByIdResponse }
            } = this.props;
        ...
        if (sect === "siteinfo") {
            ...
            let {
                electricReducer: { getSiteByIdResponse }
            } = this.props;
            ...
            if (success) {
                await this.setState({
                    values: {
                        ...
                        meter_read: meter_read ? new Date(moment(meter_read, "MM-DD-YYYY hh:mm:ss A").utc().format()) : ""
    ...
    componentDidUpdate = (prevProps, prevState) => {
        ...
        if (currVal?.building_id !== prevState?.values?.building_id) {
            currVal?.building_id && this.handleAccountDetails({ building_id: currVal?.building_id, account_type: "Electricity" });
        }
        if (currVal?.account_id !== prevState?.values?.account_id) {
            currVal?.account_id && this.handleMeterDetails({ account_id: currVal?.account_id, account_type: "Electricity" });
        }
    };
    ...
    handleAccountDetails = async params => {
        ...
        let { accounts } = this.props.electricReducer?.getAccountsListResponse;
        this.setState({
            accounts
        });
    };
    ...
    handleRegionDetails = async id => {
        ...
        let { regions } = this.props.electricReducer?.getRegionListResponse;
        this.setState({
            regions
        });
    };
    ...
    handleSiteDetails = async (id, params) => {
        ...
        let { sites } = this.props.electricReducer?.getSiteListResponse;
        this.setState({
            sites
        });
    };
    ...
    handleBuildingDetails = async (id, params) => {
        ...
        let { buildings } = this.props.electricReducer?.getBuildingListResponse;
        this.setState({
            buildings
        });
    };
    handleMeterDetails = async params => {
        ...
        let { meters } = this.props.electricReducer?.getMeterListResponse;
        this.setState({
            meters
        });
    };
    ...
    validate = () => {
        ...
        delete errorParams.billing_days;
        delete errorParams.meter_read;
    ...
    addTemplate = async () => {
        ...
        if (this.validate()) {
            ...
            await handleAddMeterTemplate({
                ...
                ccf_cost: null,
                total_electric_cost: parseFloat(values?.kwh_cost) + parseFloat(values?.kw_cost)
            });
            this.setState({
                isUploading: false
            });
    ...
    calculateMmbtu = () => {
        ...
        this.setState({
            values: {
                ...values,
                mmbtu_usage: parseFloat(values?.kwh_usage) / 293.07
            }
        });
    };
                                                                        ...
                                                                        return (
                                                                            <>
                                                                                <div className="d-flex justify-content-between">
                                                                                    <h4>{item?.label}</h4>
                                                                                    <span
                                                                                        className="custom-calculate-btn"
                                                                                        onClick={() => this.calculateMmbtu()}
                                                                                    >
                                                                                        Calculate
                                                                                    </span>
                                                                                </div>
                                                                    ...
                                                                    case "date":
                                                                        return (
                                                                            <>
                                                                                <h4>{item?.label}</h4>
                                                                                <DatePicker
                                                                                    wrapperClassName="energydatePicker"
                                                                                    autoComplete={"nope"}
                                                                                    ...
                                                                                    onChange={date => {
                                                                                        this.handleChange(item?.key, date);
                                                                                    }}
                                                                                />
                                                                            </>
                                                                        );
...
const mapStateToProps = state => {
    const { electricReducer } = state;
    return { electricReducer };
};

export default withRouter(connect(mapStateToProps, { ...electricActions })(Form));
