...
class TradeInfo extends Component {
    ...
    refreshinfoDetails = async () => {
        let tradeData = await this.props.getDataById(this.props.match.params.id);
        if (tradeData && tradeData.success) {
            this.setState({
                basicDetails: {
                    name: tradeData.name,
                    display_name: tradeData.display_name,
                    description: tradeData.description,
                    narrative_required: tradeData.narrative_required,
    ...
    restoreLogOnConfirm = async () => {
        ...
        await this.props.HandleRestoreTradeLog(selectedLog);
        this.setState({
            showConfirmModalLog: false,
            selectedLog: null,
            isHistory: false
                    ...
                    <div className="tab-dtl region-mng">
                        ...
                        {isHistory && tab === "basicdetails" ? (
                            ...
                            <div className="tab-active p-3 recomdn-table">
                                {localStorage.setItem("dynamicUrl", `/master_trades/${id}/narrative_templates`)}
                                <NarrativeTemplate hasAssign={false} dynamicUrl={`/master_trades/${id}/narrative_templates`} />
                            </div>
                        ) : tab === "assignedtabletemplate" ? (
                            <div className="tab-active p-3 recomdn-table">
                                {localStorage.setItem("dynamicUrl", `/master_trades/${id}/table_templates`)}
                                <TableTemplate hasAssign={false} dynamicUrl={`/master_trades/${id}/table_templates`} />
                            </div>
                        ) : tab === "assignedreportnotetemplate" ? (
                            <div className="tab-active p-3 recomdn-table">
                                {localStorage.setItem("dynamicUrl", `/master_trades/${id}/report_note_templates`)}
                                <ReportNoteTemplate hasAssign={false} dynamicUrl={`/master_trades/${id}/report_note_templates`} />
                            </div>
                        ) : (
