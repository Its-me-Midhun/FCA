...
class CaptionChangeModal extends Component {
    render() {
        const { onCancel, onSelection, message, buttonYes, buttonNo, subMessage, hasCancelButton, subMessage1 } = this.props;
                            ...
                            <div class="modal-body region-otr">
                                ...
                                <div class="btnOtr d-flex flex-wrap">
                                    <button type="button" class="btn btn-primary ml-2" onClick={() => onSelection(buttonYes.value)}>
                                        <span className="btn-highlight">{buttonYes.label}</span>
                                        {buttonYes.note}
                                    </button>
                                    <button type="button" class="btn btn-primary ml-2" onClick={() => onSelection(buttonNo.value)}>
                                        <span className="btn-highlight">{buttonNo.label}</span>
                                        {buttonNo.note}
                                    </button>
                                    {hasCancelButton && (
...
export default CaptionChangeModal;
