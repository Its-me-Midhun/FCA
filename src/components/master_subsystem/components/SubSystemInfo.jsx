...
class SubSystemInfo extends Component {
    state = {
        isloading: true,
        basicDetails: {
            name: "",
    ...
    refreshinfoDetails = async () => {
        let subSystemData = await this.props.getDataById(this.props.match.params.id);
        if (subSystemData && subSystemData.success) {
            this.setState({
                basicDetails: {
                    name: subSystemData.name,
                    trade: subSystemData.trade,
                    system: subSystemData.system,
                    display_name: subSystemData.display_name,
                    description: subSystemData.description,
                    narrative_required: subSystemData.narrative_required,
                    created_at: subSystemData.created_at,
                    updated_at: subSystemData.updated_at,
                    benchmark: subSystemData.benchmark,
                    service_life: subSystemData.service_life
                },
                isloading: false,
                isHistory: false
            });
    ...
    restoreLogOnConfirm = async () => {
        ...
        await this.props.HandleRestoreSubSystemLog(selectedLog);
        this.setState({
            showConfirmModalLog: false,
            selectedLog: null,
            isHistory: false
    ...
    updateBenchmark = (params) => {
        this.props.handleUpdateBenchmark(this.props.match.params.id, { sub_system: params });
        this.setState({ basicDetails: { ...this.state.basicDetails, benchmark: params.benchmark } });
    };
                    ...
                    <div className="tab-dtl region-mng">
                        ...
                        {isHistory && tab === "basicdetails" ? (
                            ...
                            <BasicDetails
                                ...
                                entity={entity}
                                updateBenchmark={this.updateBenchmark}
                            ...
                            <div className="tab-active p-3 recomdn-table bg-grey-table ">
                                {localStorage.setItem("dynamicUrl", `/master_sub_systems/${id}/narrative_templates`)}
                                <NarrativeTemplate hasAssign={false} dynamicUrl={`/master_sub_systems/${id}/narrative_templates`} />
                            </div>
                        ) : tab === "assignedtabletemplate" ? (
                            <div className="tab-active p-3 recomdn-table bg-grey-table">
                                {localStorage.setItem("dynamicUrl", `/master_sub_systems/${id}/table_templates`)}
                                <TableTemplate hasAssign={false} dynamicUrl={`/master_sub_systems/${id}/table_templates`} />
                            </div>
                        ) : tab === "assignedreportnotetemplate" ? (
                            <div className="tab-active p-3 recomdn-table bg-grey-table">
                                {localStorage.setItem("dynamicUrl", `/master_sub_systems/${id}/report_note_templates`)}
                                <ReportNoteTemplate hasAssign={false} dynamicUrl={`/master_sub_systems/${id}/report_note_templates`} />
                            </div>
                        ) : tab === "assignedrecommendationtemplate" ? (
                            <div className="tab-active p-3 recomdn-table bg-grey-table">
                                {localStorage.setItem("dynamicUrl", `/master_sub_systems/${id}/recommendation_templates`)}
                                <RecommendationTemplate hasAssign={false} dynamicUrl={`/master_sub_systems/${id}/recommendation_templates`} />
                            </div>
                        ) : (
...
export default withRouter(SubSystemInfo);
