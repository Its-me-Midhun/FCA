...
class TableTemplateInfo extends Component {
    state = {
        isloading: true,
        basicDetails: {
            name: "",
    ...
    refreshinfoDetails = async () => {
        let tableTemplateData = await this.props.getDataById(this.props.match.params.id);
        if (tableTemplateData && tableTemplateData.success) {
            this.setState({
                basicDetails: {
                    name: tableTemplateData.name,
                    html_format: tableTemplateData.html_format,
                    double_header: tableTemplateData.double_header,
                    footer: tableTemplateData.footer,
                    description: tableTemplateData.description,
                    created_at: tableTemplateData.created_at,
                    updated_at: tableTemplateData.updated_at
                },
                isloading: false,
                isHistory: false
            });
    ...
    restoreLogOnConfirm = async () => {
        ...
        await this.props.HandleRestoreTableTemplateLog(selectedLog);
        this.setState({
            showConfirmModalLog: false,
            selectedLog: null,
            isHistory: false
                    ...
                    <div className="tab-dtl region-mng">
                        ...
                        {isHistory && tab === "basicdetails" ? (
                            ...
                            <BasicDetails
                                keys={keys}
                                config={config}
                                basicDetails={{ ...basicDetails, useTextEditor: true }}
                                handleDeleteItem={handleDeleteItem}
                                isHistoryView={true}
                                changeToHistory={this.changeToHistory}
                                isHistory={isHistory}
...
export default withRouter(TableTemplateInfo);
