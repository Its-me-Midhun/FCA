...
class Form extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            meterTemplate: [
                ...
                { key: "building_id", type: "id", arr: "buildings", label: "Select Building", disabled: disabledBuilding.includes(this.props.sect) }
            ],
            subTemplates: [
    ...
    componentDidMount = async () => {
        ...
        if (this.props.selectedMeterTemplate) {
            ...
            const {
                electricReducer: {
                    getMeterTemplateByIdResponse: { success, region, site, building, year: yearId, rating }
                }
            } = this.props;
            if (success) {
                await this.setState({
    ...
    componentDidUpdate = (prevProps, prevState) => {
        ...
        if (currVal?.building_id !== prevState?.values?.building_id) {
            currVal?.building_id && this.handleAccountDetails({ building_id: currVal?.building_id });
        }
        if (currVal?.account_id !== prevState?.values?.account_id) {
            currVal?.account_id && this.handleMeterDetails({ account_id: currVal?.account_id });
        }
    };
    ...
    handleChange = (key, val) => {
        ...
        switch (key) {
            ...
            case "building_id":
                ...
                break;
            case "account_id":
                this.setState({ values: { ...tempVal, account_id: val, meter_id: "" } });
                break;
            default:
                                                            ...
                                                            <div className="custom-selecbox">
                                                                ...
                                                                >
                                                                    ...
                                                                    {this.state?.[item?.arr]?.map(x => (
                                                                        <option value={x?.id}>{x?.name || x?.number || x?.meter}</option>
                                                                    ))}
                                                                </select>
                                                            </div>
                                                        </div>
