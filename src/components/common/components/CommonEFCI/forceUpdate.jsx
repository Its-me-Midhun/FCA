                            ...
                            <div class="modal-body region-otr build-type-mod">
                                                                    ...
                                                                    <div className="pos-sec">
                                                                        ...
                                                                        <>
                                                                            <NumberFormat
                                                                                ...
                                                                                className={`form-control fc-no-dot ${setColor(efciData.expected_fcis[index].value) ? "text-light" : ""} ${(this.props.disableClick || (isChecked && isChecked.length && isChecked.find(i => i == item.index) ? false : true)) ? "cursor-notallowed" : ""}`}
                                                                                ...
                                                                                onValueChange={async values => {
                                                                                    const { value } = values;
                                                                                    return await this.props.forceUpdateData(value, efciData.expected_fcis[index].index, index);
                                                                                ...
                                                                                onKeyPress={async event => {
                                                                                    if (event.key === "Enter") {
                                                                                        await this.setState({
                                                                                            ...
                                                                                            await this.props.forceUpdateData(this.state.target.value, efciData.expected_fcis[index].index, index);
                                                                                        });
                                                                            ...
                                                                            // onBlur={async () => {
                                                                            //     await this.props.forceUpdateData(efciData.expected_fcis[index].id, this.state.initialEfci ,index);
                                                                            //     await this.setState({
                                                                            //         selectedEfci: null,
                                                                            //         initialEfci: null
                                                                            //     });
                                ...
                                {tempArray &&
                                    tempArray.length ? <div class="text-center btnOtr mt-2 mb-3" onClick={() => this.props.onCancel()}><button type="button" class="btn btn-secondary btnClr col-md-6" data-dismiss="modal">Cancel</button>
                                        <button type="button" class="btn btn-primary btnRgion col-md-6" onClick={() => this.props.saveData()} disabled={!this.props.isValueChanged}>Save</button></div> : null}
                            </div></div>
                    </div>
                </LoadingOverlay>
            </div>
