...
export const LockUnlock = ({ locked, lockProject, partial_locked }) => {
    useEffect(() => {
    ...
    return (
        <>
            <ReactTooltip id="lockUnlock" place="bottom" effect="solid" className="rc-tooltip-custom-class" backgroundColor="#007bff" />
            {partial_locked && (
                <div
                    data-for="lockUnlock"
                    data-tip="Some child entities are locked and some are unlocked. To lock, click the unlock button."
                    className={`locked-section `}
                >
                    <button className={`add-btn add-build-btn btn-prt-lck`}>
                        <span className="txt-spn">Partially Locked</span>
                    </button>
                </div>
            )}
            {locked ? (
                <div data-for="lockUnlock" data-tip="Click to Unlock" className={`locked-section ${locked === true ? "green-active active" : ""}`}>
                    <button className="add-btn add-build-btn locked" onClick={() => lockProject(false)}>
                        <LockIcon />
                        <span className="txt-spn"> </span>
                    </button>
                </div>
            ) : (
                <div data-for="lockUnlock" data-tip="Click to Lock" className={`locked-section unlock-sec ${locked === false ? "active" : ""}`}>
                    <button className={`add-btn add-build-btn ${locked === false ? "unlocked" : ""}`} onClick={() => lockProject(true)}>
                        <UnlockIcon />
                        <span className="txt-spn"> </span>
                    </button>
                </div>
