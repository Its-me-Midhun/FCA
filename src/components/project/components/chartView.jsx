...
class ChartView extends Component {
    ...
    renderChartData = () => {
        const { graphData, viewContent, currentTab, funding_options, capital_spending_plans, filterView, chartType } = this.props;
        ...
        if (viewContent === "totalView" && currentTab !== "csp&efci") {
            ...
            if (filterView === "efci" || filterView === "both") {
                funding_options.forEach((fo, key) => {
                    legendArray.push({
                        name: fo.index,
                        order: fo.order,
            ...
            if (funding_options && funding_options.length) {
                lineChartValue = funding_options.map((fo, key) => {
                    const annualFundingOptions = fo.annual_funding_options?.map(am => parseFloat(am.amount)) || [];
                    const annualEfcis = fo.annual_efcis?.map(am => parseFloat(am.value)) || [];
                    const totalAmount = annualFundingOptions.reduce((total, obj) => obj + total, 0);
                    ...
                    return {
                        ...
                        // dataLabels: {
                        //     style: { color: this.renderColors(key) },
                        //     enabled: true,
                        //     allowOverlap: false,
                        //     borderRadius: 5,
    ...
    renderFundingCostEfciRestoreModalLog = () => {
        const { openFundingCostEfciRestore, changeSet } = this.state;
    ...
    renderFundingTotalFundingCostRestoreModalLog = () => {
        const { openTotalFundingCostRestore, changeSet } = this.state;
    ...
    renderProjectFundingEfciRestoreModalLog = () => {
        const { openFundingEfciLogRestore, changeSet } = this.state;
    ...
    exportChartFromServer = async (type, noOfYears) => {
        try {
            if (this.chartRef) {
                ...
                const project_id = this.props.dataView == "project" ? this.props.projectId : url.searchParams.get("pid");
                ...
                currentTab === "system" && formData.append("trade_ids", JSON.stringify([selectedSystem.id]));
                let filter = {};
                switch (dataView) {
                    case "building":
    ...
    exportDataTableToWord = async file_type => {
        try {
            this.setState({ isExporting: true });
                    ...
                    <SelectExportTypeWordModal
                        ...
                        onOk={(sort_type, file_type) => {
                            ...
                            if (currentTab === "csp&efci") {
                                ...
                                this.exportDataTableToWord(file_type);
                            }
                        }}
                    />
                }
    ...
    render() {
        ...
        const { dataSource, totalAmount, legendValues, legendArrayFunding, isOpenColorCode, isExporting } = this.state;
        let labelValues = graphData && graphData.length ? graphData.map(gd => gd.data) : [];
        let mergedArray = [].concat.apply([], labelValues);
        var holder = {};
        mergedArray.forEach(function (d) {
        ...
        let noOfYears = endYear - startYear + 1 || 0;
        let filterCount = 0;
        filterValues &&
                ...
                {currentTab !== "csp&efci" ? (
                    <div className="tab-active">
                        <div className="chart-right-flt fil-item" id={"selectDropdownParent"}>
                            {graphData && graphData.length ? (
                                <div className="hed-section-gr col-md-6">
                                    <h1 className="line">
                                        <span>
                                            {projectData && projectData.project_name ? `${projectData.project_name} - ` : ""}{" "}
                                            {projectData && projectData.name}{" "}
                        ...
                        <div className={`chart-area ${currentTab === "all" ? "chart-coming-soon" : ""}`} id={"chartItem"}>
                            {currentTab !== "all" ? (
                                <>
                    ...
                    <div className="tab-active pb-0">
                        <div className="chart-right-flt btnPsn lgd-otr">
                            ...
                            {isOpenColorCode ? (
                                <>
                                    <ColorCodeData isOpenColorCode={isOpenColorCode} colorCodes={this.props.colorCodes} />
                                </>
                            ) : null}
