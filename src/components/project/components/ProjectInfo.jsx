...
import Regions from "../../region/index";
...
import ViewImportHistoryTable from "../../recommendations/components/ViewImportHistoryTable";
...
class ProjectInfo extends Component {
    state = {
        ...
        paginationParamsSystem: this.props.projectReducer.entityParams.paginationParams,
        paramsSystem: this.props.projectReducer.entityParams.params,
        paginationParamsDepartment: this.props.projectReducer.entityParams.paginationParams,
        paramsDepartment: this.props.projectReducer.entityParams.params,
        isHistory: false,
    ...
    componentDidMount = async () => {
        ...
        this.props.showInfoPage(id);
        this.refreshinfoDetails();
        if (tab === "settings") {
            this.getBuildingTypeSettingsData();
        }
    };
        ...
        if (projectData && projectData.success) {
            this.setState({
                basicDetails: {
                    ...
                    client_users: projectData.client_users,
                    consultancy: projectData.consultancy,
                    color_code: projectData.color_code,
                    locked: projectData.lock_status === LOCK_STATUS.LOCKED,
                    partial_locked: projectData.lock_status === LOCK_STATUS.PARTIALLY_LOCKED
                },
                isloading: false,
                isHistory: false
            });
    ...
    getBuildingTypeSettingsData = async () => {
    ...
    render() {
        ...
        // console.log("this.props.efciData", this.props.efciData);
        return (
            <React.Fragment>
                <ReactTooltip id={`init-sp-rep-btn`} />
                <div className="dtl-sec system-building col-md-12">
                    <div className="tab-dtl region-mng">
                        <InfoTabs infoTabsData={infoTabsData} isTabClass={true} isTabWideClass={true} />
                        {tab === "basicdetails" ? (
                            <HelperIcon
                                isHistory={isHistory}
                                entity={isHistory ? "general" : entity}
                                additionalSpanClass={hasLock ? "help-top" : ""}
                            />
                        ) : null}
                        {/* {isInitializingSpecialReport ? (
                            <button className="add-btn upload-button init-sp-rep-btn">
                            ...
                            <button
                                className="add-btn upload-button init-sp-rep-btn"
                                onClick={() => this.initializeSpecialReport(this.props.projectId)}
                                title="Sync Reports"
                            >
                                <i class="fas fa-sync"></i>
                            </button>
                        )} */}
                        {/* {!query.isImageView && hasEdit && (
                            <button className="add-btn upload-button" onClick={() => showUploadDataModal(id)} title="Upload">
                        ...
                        {isHistory && tab === "basicdetails" ? (
                            ...
                            <BasicDetails
                                ...
                                hasLock={hasLock}
                                hasUpload={!query.isImageView && hasEdit}
                                showUploadDataModal={() => showUploadDataModal(id)}
                            />
                        ) : tab === "efci" ? (
                            <EFCIMain
                            ...
                            <ProjectSettings
                                ...
                                subsystemTableData={subsystemTableData}
                                ...
                                params={params}
                                handlePageClickSystem={this.handlePageClickSystem}
                                handlePerPageChangeSystem={this.handlePerPageChangeSystem}
                                handlePageClickDepartment={this.handlePageClickDepartment}
                                handlePerPageChangeDepartment={this.handlePerPageChangeDepartment}
                                paginationParamsSystem={paginationParamsSystem}
                                paramsSystem={paramsSystem}
                                paginationParamsDepartment={paginationParamsDepartment}
                                paramsDepartment={paramsDepartment}
                                handleAddLimit={handleAddLimit}
                                getItem={getItem}
                                getItembyId={this.getTradeById}
                                updateData={updateData}
                                refreshset={this.refreshinfoDetails}
                                ...
                                handleSelectTrue={this.handleSelectTrue}
                                handleSelectRow={this.handleSelectRow}
                                handleColumnselect={this.handleColumnselect}
                                revertBuildingData={this.revertBuildingData}
                                // ---------colorcodelog-----------------
                            ...
                            <>
                                <ProjectReports isReportView={isReportView} spReportEntityData={spReportEntityData} />
                            </>
                        ) : tab === "regions" ? (
                            <div className="tab-active recomdn-table bg-grey-table">
                                <Regions projectId={this.props.match.params.id} basicDetails={basicDetails} />
                            </div>
                        ) : tab === "sites" ? (
                            <div className="tab-active recomdn-table bg-grey-table">
                                <Sites projectId={this.props.match.params.id} isProjectView={true} basicDetails={basicDetails} />
                            </div>
                        ) : tab === "buildings" ? (
                            <div className="tab-active recomdn-table bg-grey-table">
                                <Buildings projectId={this.props.match.params.id} basicDetails={basicDetails} />
                            </div>
                        ) : tab === "history" ? (
                            <div className="tab-active  recomdn-table bg-grey-table">
                                <ImportHistory
                                    getAllImportHistoryLogs={getAllImportHistoryLogs}
                                    ...
                                    importHistoryParams={importHistoryParams}
                                    handleGlobalSearchimportHistory={handleGlobalSearchimportHistory}
                                    globalSearchKeyimportHistory={globalSearchKeyimportHistory}
                                    handleDownloadItem={handleDownloadItem}
                                    importhistoryPaginationParams={importhistoryPaginationParams}
                                    handlePerPageChangeImportHistory={handlePerPageChangeImportHistory}
                                    handlePageClickImportHistory={handlePageClickImportHistory}
                                    updateImportTableSortFilters={updateImportTableSortFilters}
                                    resetAllImportFilters={resetAllImportFilters}
                                    resetImportSort={resetImportSort}
                                    showViewImportModal={showViewImportModal}
                                    toggleImportWildCardFilter={toggleImportWildCardFilter}
                                    showImportWildCardFilter={showImportWildCardFilter}
                                    updateImportWildCardFilter={updateImportWildCardFilter}
                                    exportImportTableXl={exportImportTableXl}
                            ...
                            <div className="tab-active recomdn-table bg-grey-table">
                                <ViewImportHistoryTable projectId={this.props.match.params.id} />
                            </div>
                        ) : tab === "documents" ? (
                            <div className="tab-active recomdn-table bg-grey-table">
                                <Documents projectId={this.props.match.params.id} basicDetails={basicDetails} />
                            </div>
                        ) : (
                            <div className="tab-active recomdn-table bg-grey-table pb-0">
                                <Recommendations basicDetails={basicDetails} projectId={this.props.match.params.id} />
                            </div>
                        )}
                    </div>
                </div>
